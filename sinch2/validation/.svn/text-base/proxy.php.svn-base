...
else
{
    ...
    if ($_flags['include_form'] && !isset($_GET['nf']))
    {
        $_url_form      = '<div style="width:100%;margin:0;text-align:center;border-bottom:1px solid #725554;color:#000000;background-color:#F2FDF3;font-size:12px;font-weight:bold;font-family:Bitstream Vera Sans,arial,sans-serif;padding:4px;">'
			...
			. ' <input type="hidden" id="real_url" value ="' . $_url . '" />'
    ...
    // inject some javascript to pass the url to the sinch page every time the proxy reloads
    $_sinch_head = '<script type="text/javascript" src="http://jqueryjs.googlecode.com/files/jquery-1.3.2.min.js"></script>'
    		 . '<script type="text/javascript" src="proxy.js"></script>'
		 . '<script type="text/javascript">var real_url = "' . $_url . '";</script>';
    $_response_body = preg_replace('#\<\s*head(.*?)\>#si', "$0\n$_sinch_head" , $_response_body, 1);
}
